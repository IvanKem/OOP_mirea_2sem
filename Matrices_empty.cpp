...
class InvalidSumException : public Exception {
public:
    InvalidSumException(const char* s) : Exception(s) {}
    InvalidSumException(const InvalidOperationException& e) : Exception(e) {}

    virtual void print() {
        cout << "InvalidSumException: " << str << what();
    }
};
...
public:
    ...
    double* Sum_of_Rows_vector()
    {
        double* res = new double[BaseMatrix<T>::width];
        for (int i = 0; i < BaseMatrix<T>::height; i++){
            for (int j = 0; j < BaseMatrix<T>::width;j++)
            {
                res[i]+= BaseMatrix<T>::ptr[i][j];
            }
            if (res[i]==0){throw InvalidSumException("InvalidSumException in BaseMatrix");}
        }
